...
export const generateAnalysisPDF = (data, fileName = 'Resume') => {
    try {
        ...
        // ===== RISK ANALYSIS =====
        const riskAnalysis = data.riskAnalysis || data.risk_analysis || (data.analysis && (data.analysis.riskAnalysis || data.analysis.risk_analysis)) || {};

        if (riskAnalysis.overall_risk_score !== undefined) {
            addSectionHeader('🛡️ Risk Evaluation');

            // Overall Risk
            doc.setFontSize(11);
            doc.setFont('helvetica', 'bold');
            doc.text('Risk Level: ', 20, yPosition);
            doc.setFont('helvetica', 'normal');
            
            const riskLevel = riskAnalysis.risk_level || 'Unknown';
            const riskColor = riskLevel === 'High' ? [239, 68, 68] : riskLevel === 'Medium' ? [245, 158, 11] : [34, 197, 94];
            doc.setTextColor(...riskColor);
            doc.text(riskLevel, 60, yPosition);
            doc.setTextColor(0, 0, 0);
            yPosition += 8;

            doc.setFontSize(11);
            doc.setFont('helvetica', 'bold');
            doc.text('Risk Score: ', 20, yPosition);
            doc.setFont('helvetica', 'normal');
            doc.text(`${riskAnalysis.overall_risk_score}/100`, 60, yPosition);
            yPosition += 12;

            // Project Authenticity
            if (riskAnalysis.project_analysis) {
                const proj = riskAnalysis.project_analysis;
                doc.setFontSize(12);
                doc.setFont('helvetica', 'bold');
                doc.text('Project Authenticity Analysis:', 20, yPosition);
                yPosition += 8;

                doc.setFontSize(10);
                doc.setFont('helvetica', 'normal');
                doc.text(`Score: ${proj.authenticity_score}%`, 25, yPosition);
                yPosition += 6;
                doc.text(`Detail Level: ${proj.detail_level}`, 25, yPosition);
                yPosition += 6;
                
                if (proj.observation) {
                    const obsLines = doc.splitTextToSize(proj.observation, pageWidth - 45);
            ...
            // Red Flags
            if (riskAnalysis.red_flags && riskAnalysis.red_flags.length > 0) {
                doc.setFontSize(12);
                doc.setFont('helvetica', 'bold');
                doc.text('Detected Red Flags:', 20, yPosition);
                yPosition += 8;

                riskAnalysis.red_flags.forEach((flag, idx) => {
                    checkPageBreak(15);
                    doc.setFontSize(10);
                    doc.setFont('helvetica', 'bold');
                    doc.setTextColor(239, 68, 68);
                    ...
                    doc.setTextColor(0, 0, 0);
                    doc.text(`${flag.category} (${flag.severity})`, 28, yPosition);
                    yPosition += 5;
                    
                    doc.setFont('helvetica', 'normal');
                    const descLines = doc.splitTextToSize(flag.description, pageWidth - 50);
                    doc.text(descLines, 28, yPosition);
